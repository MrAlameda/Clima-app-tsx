...
const BoxClima=()=>{
    ...
    const cityGeoLocation=(a:string)=>{
        axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${a}&appid=93ad85c660dbccd5a0e37e8846eb804c`)
            .then(res=>{
                ...
                setTemp({
                    ...temp,
                    tempMax:Number(res?.data.main.temp_max),
                    tempMin:Number(res?.data.main.temp_min),
                    type:"-F"
                })
            })
            .catch(err=>{
    ...
    const geoLocation=(lat:number, lon:number)=>{
